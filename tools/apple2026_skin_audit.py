...
CLAIM_CONTRACTS = {
    "Apple2026.sbs": {
        "required": [
            ...
            ("%Vl(hdr_title_split,10,0,44,20,8)", "split header title is left aligned in the left column"),
            ...
            ("%?pv<%xd(Ca)|%xd(Cb)|%xd(Cc)>", "quickscreen speaker must follow the volume level"),
    ...
    "Apple2026.wps": {
        "required": [
            ...
            ("%Vl(repeat_icon_state,20,206,15,15,-)", "repeat state sits bottom-left of the transport"),
            ("%?mm<||||%acA-B>", "A-B repeat must be named, it has no glyph of its own"),

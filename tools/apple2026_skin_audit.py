...
CLAIM_CONTRACTS = {
    "Apple2026.sbs": {
        "required": [
            ...
            ("%Vl(lock_split,110,4,9,12,-)", "split bars need their own lock slot"),
            ("%Vl(hdr_title,10,0,94,20,3)", "full-screen header title must stop short of the clock viewport"),
            ("%Vl(hdr_title_split,10,0,54,20,3)", "split header title is left aligned in the left column"),
            ...
            ("%Vl(hdr_clock_split,66,0,42,20,8)", "clock is centred on the left panel for split shells"),

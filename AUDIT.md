...
|---|---|---|---|---|
| 2026-08-10 | F-A1 (PLAN.md) | **H-27**: deriva del panel recalibrada contra el firmware original — 18 s → 7 s, 4 diagonales de 45° → 8 direcciones (4 cardinales + 4 diagonales a 60°/120°/240°/300°). `pan_pos_now_fp()` reescrito para holgura arbitraria por dirección; sólo el eje dominante se compone subpíxel, a propósito, para no triplicar el coste sin haberlo medido en aparato. Matemática verificada por réplica independiente (Python) para las 8 direcciones y por traza en vivo para el caso cardinal. Ambos targets compilan. | Sólo se vio UNA dirección rodando en simulador — límite ya conocido (`storage_disk_is_active()` nunca da true ahí), no observable sin aparato. F-A3..F-A6 y B-0..B-5 de `PLAN.md` sin empezar. | Confirmar en aparato las 8 direcciones con el disco girando de verdad; seguir con F-A2 (gramática de la barra: reloj con revelado al mantener SELECT) |

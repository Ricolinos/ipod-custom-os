...
### H-28 · F-A4: prueba de concepto de la transición de empuje (Música → Canciones)
- Estado: **hecho; matemática verificada por traza repetida, animación en movimiento razonado-no-observado** (2026-08-10) · Fuente: `PLAN.md` §0.5, investigación UX/UI del firmware original, geometría confirmada por el usuario ante ambigüedad de la nota original
- **La investigación tenía una contradicción literal** que había que resolver antes de escribir una línea: "se desplaza de izquierda a derecha... lo empuja hacia afuera de la pantalla del lado izquierdo" — algo que se mueve hacia la derecha no puede empujar nada hacia la izquierda. Pregunta directa al usuario con dos maquetas ASCII; confirmó: la pantalla nueva entra por la **derecha**, avanza hacia la **izquierda**, cubre primero el panel de imagen y por último expulsa la lista por el borde izquierdo.
- **Hallazgo de arquitectura, no de bug**: el cambio de vista dividida a completa no pasa por ningún gancho en C — vive entero en el `.sbs` (`%?Lo`/`%?LM` comparando el título de la lista) y se resuelve con un redibujo instantáneo del motor de skins. No había nada que "interceptar" a medio camino.
- **Diseño elegido, y por qué NO toca `tree.c`**: capturar la pantalla saliente (`lcd_framebuffer_default.fb_ptr`, el mismo mecanismo que ya usa `skin_backdrops.c`) justo antes de navegar; dejar que la lista de destino se dibuje por su camino de siempre, sin tocarlo; en el primer redibujo de esa lista (enganchado en `list.c`, mismo sitio que el panel) tapar la pantalla entera con la captura y retirarla en franjas de derecha a izquierda. `tree.c`, compartido con el navegador de archivos y otros plugins, queda intacto — todo el riesgo nuevo vive en un módulo propio (`apple2026_transition.c`).
- **Trampa que casi cuela**: `apple2026_transition.h` sólo incluía `config.h` antes de comprobar `#if ROCKPOD_APPLE2026_IPOD` — exactamente la trampa que ya documentó `usb_screen.c` ("es esta cabecera la que define la macro; comprobarla antes de incluirla hace que nunca se incluya"). El enlazador lo delató de inmediato: dos símbolos indefinidos. Arreglado incluyendo `apple2026_shell.h` sin condición.
- Memoria: 150 KB pedidos con `core_alloc`/`core_pin` justo para la transición, liberados al terminar — no reservados a fuego como `pane_workbuf`, porque esto se usa unos cientos de milisegundos y rara vez.
- Acotado a **una sola vista** (Canciones, `param==0` en `db_view_fn`) a propósito: validar el mecanismo antes de conectarlo a las otras cinco vistas del mismo submenú.
- **Puerta `lcd_active()` añadida en esta misma sesión, no en el primer commit**: la primera versión no la llevaba — un descuido, no una decisión. Con la pantalla dormida ahora se suelta el búfer sin animar, en vez de gastar ~400 ms de CPU en un barrido que nadie ve. Regla no negociable del proyecto (CLAUDE.md, "Energía").
- **Verificación — la parte metodológicamente interesante**: intentar grabar el simulador en vídeo (con permiso explícito del usuario) para ver el barrido reveló una cadena de obstáculos, ninguno relacionado con el código de la transición:
  1. El volcado F5 se sirve en cola y sólo se procesa cuando `pump()` (que bloquea todo el hilo de UI) termina — así que toda captura por volcado, sin importar cuánto durase la animación, mostraba siempre el estado final.
  2. `ffmpeg`/AVFoundation tarda varios segundos en "calentar" el dispositivo de captura antes de grabar fotogramas reales; grabar y disparar casi a la vez pierde la transición entera.
  3. `sleep()` en este simulador **no se corresponde de forma fiable con tiempo real** — la misma constante (`HZ*10`) produjo duraciones observadas muy distintas entre corridas. Es el mismo tipo de imprecisión que CLAUDE.md ya documenta para la rueda ("sin velocímetro cae a cadencia de eventos").
  4. SELECT es un botón sondeado: dos pulsaciones sintéticas demasiado seguidas pierden la segunda: en una corrida, la segunda pulsación quedó en cola durante todo el bloqueo de `pump()` y sólo se consumió al terminar, seleccionando y reproduciendo una canción — lo cual, de paso, confirma que `pump()` sí bloquea el hilo entero tal como está diseñado.
- Dado este límite —no del código, del arnés— la verificación se apoyó en lo que SÍ es fiable: **tres corridas de traza independientes**, todas con `cover_w` bajando exactamente 320→288→...→0 en 11 pasos, sin fallos; y **captura del estado final** en cada intento, siempre limpio (lista completa, sin artefactos, sin cuelgue), con la temporización real (400 ms) tras revertir toda instrumentación temporal.
- Pendiente de aparato/usuario: **[~]** ver el barrido en movimiento con los propios ojos (el usuario sí puede, en tiempo real, donde este arnés no puede) y medir consumo — es exactamente la condición que F-A4 exige antes de aprobar la familia héroe, más cara.

### H-27 · F-A1: deriva del panel recalibrada contra el firmware original (7 s, 8 direcciones)
- Estado: **hecho; matemática verificada por réplica independiente + traza en vivo, rotación completa en simulador razonado-no-observado** (2026-08-10) · Fuente: `PLAN.md` §0.3, investigación UX/UI del firmware original
- No era un bug: era una recalibración pedida tras medir el original. Dos números y un mecanismo cambian:
  1. **`PANE_HOLD_TICKS`: 18 s → 7 s** (`apple2026_pane.c:77`), igualando "cada álbum se muestra durante 7 segundos" del original.
  2. **4 diagonales de 45° → 8 direcciones** (`PAN_DIRECTIONS[8]`, `:166-176`): las 4 cardinales (arriba/abajo/izquierda/derecha) más 4 diagonales pronunciadas a 60°/120°/240°/300° — no a 45°. El documento de investigación no dice a qué cuadrante corresponde cada ángulo, sólo el grado; se asignaron simétricos (60°/120° hacia arriba, 240°/300° hacia abajo) por ser la única asignación que no privilegia un lado — **decisión de diseño propia, no del original**, señalada como tal.
- `pan_pos_now_fp()` reescrito: antes asumía recorrido IGUAL en ambos ejes (por diseño, sólo servía a 45°); ahora calcula el recorrido máximo `L` que cabe en la holgura para la dirección real (`L = min(rx·256/|dx|, ry·256/|dy|)`) y compone la posición como una recta centrada, en punto fijo 8.8.
- **Decisión de coste, tomada sin pedir la prueba de aparato**: con direcciones arbitrarias los dos ejes ya no avanzan a la misma velocidad (128 contra 222 en las diagonales de 60°, por ejemplo). Componer subpíxel LOS DOS ejes exige un filtro bilineal — 3 mezclas por píxel en vez de 1, TRIPLICANDO el coste de este paso (que H-16 dejó en la mitad del fundido/segundo). Eso habría entrado justo en el terreno que F-A4 reserva para medir en el aparato antes de aprobar. En su lugar, sólo se compone subpíxel el **eje dominante** (`|dx|` o `|dy|` mayor); el eje menor avanza en píxeles enteros sin mezcla — mismo coste de composición que antes de este cambio, cero riesgo nuevo de consumo. Si en el aparato se nota un escalón en el eje menor de las diagonales, la salida documentada es subir a bilineal ahí, no bajar la cadencia.
- **Verificación**: réplica independiente en Python de la fórmula de punto fijo para las 8 direcciones contra `rx=128, ry=48` (los valores reales vistos en la traza) — todos los extremos caen dentro de límites, y el eje sin componente (`dx=0` o `dy=0`) queda exactamente fijo en el centro en las tres muestras de tiempo comprobadas. Traza en vivo (`A26_PAN_TRACE`, temporal) confirmó el caso cardinal (dir=2, vertical arriba) pixel a pixel: x fijo en 64 durante todo el recorrido, y descendiendo de 48 a 0, tap correcto (`ty=-1` en marcha, `ty=0` al llegar al borde).
- **Límite del simulador, no del código**: sólo se pudo observar UNA dirección en vivo (18,75 s → 7,75 s no cambia el motivo ya documentado en CLAUDE.md: `storage_disk_is_active()` nunca da `true` en el simulador, así que el slot trasero nunca precarga y el pase se queda clavado en la primera carátula tras su único recorrido). Las 7 direcciones restantes están verificadas por la réplica matemática, no vistas rodando. Pendiente de aparato: **[~]** confirmar a ojo que las 8 direcciones aparecen con el disco realmente girando, y que el eje menor de las diagonales no se ve a saltos.
- Ambos targets compilan limpio.
...
|---|---|---|---|---|
| 2026-08-10 | F-A1 + F-A4 (PLAN.md) | **H-27**: deriva del panel recalibrada — 18 s → 7 s, 4 diagonales de 45° → 8 direcciones. **H-28**: prueba de concepto de la transición de empuje (Música → Canciones), sin tocar `tree.c`, con puerta `lcd_active()`. Ambas verificadas por traza/réplica matemática; ambos targets compilan en todo momento. Se intentó grabar el simulador en vídeo (permiso explícito del usuario) para ver el empuje en movimiento: reveló que `sleep()` no es fiable como reloj en este simulador, que F5 se sirve en cola al terminar `pump()`, y que SELECT sondeado pierde pulsaciones muy seguidas — ninguno de los tres es un fallo del código, los tres quedan documentados en H-28 para la próxima sesión que intente algo similar. | Ninguna de las dos animaciones se vio EN MOVIMIENTO por esta sesión — límite del arnés, no del código. F-A2, F-A3, F-A5, F-A6 y B-0..B-5 de `PLAN.md` sin empezar. Empuje acotado a una sola vista (Canciones) de seis. | El usuario mira ambas animaciones con sus propios ojos en el simulador (que sí puede, en tiempo real); si se ven bien, extender el empuje a las otras 5 vistas y medir consumo en aparato (F-A4, autorizado) |
